...
amplitude = function () {
  ...
  p5.Amplitude.prototype._audioProcess = function (event) {
    for (var channel = 0; channel < event.inputBuffer.numberOfChannels; channel++) {
      ...
      var lastAvg = 0;
      for(var ix = 0; ix < ma; ix++){
        lastAvg+=inputBuffer[ix];
      }
      lastAvg /= ma;

      for(var ix = 0; ix < bufLength -  ma - 2; ix++){
        lastAvg += (inputBuffer[ix+ma] - inputBuffer[ix])/ma
        newInputBuffer.push(lastAvg)
      }
      inputBuffer = newInputBuffer;
      bufLength = inputBuffer.length;
